...
function searchLeave() {
    var name = $('#search_leave_name').val();
...
$(document).ready(function() {
    var columns = [
        ...
        {data: null, render: function (data, type, row, meta) {
                var content = '<div class="button-list">'+
                    '<button onclick="removeOne(\''+row.id+'\', \'leave/removeOne\')" type="button" class="btn btn-icon waves-effect waves-light btn-danger"> ' +
                        '<i class="fa fa-remove"></i> ' +
                    '</button>'+
                    '<button onclick="gotoUpdatePage(\'leave/updatePage\',\'更新节假日信息\',\''+row.id+'\')" type="button" class="btn btn-icon waves-effect waves-light btn-warning"> ' +
    ...
    table = loadTabelWithCustomerSearcher('leave/findPage', columns, 'leave/addPage', '新增节假日');
    init();
});