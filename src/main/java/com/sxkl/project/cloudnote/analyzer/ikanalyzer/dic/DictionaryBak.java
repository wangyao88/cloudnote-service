 ...
 * 词典管理类,单子模式
 */
public class DictionaryBak {

	/*
	 * 词典单子实例
	 */
	private static DictionaryBak singleton;
	...
	private DictionaryBak(Configuration cfg) {
		this.cfg = cfg;
		this.loadMainDict();
	 ...
	 * @return Dictionary
	 */
	public static DictionaryBak initial(Configuration cfg) {
		if (singleton == null) {
			synchronized (DictionaryBak.class) {
				if (singleton == null) {
					singleton = new DictionaryBak(cfg);
					return singleton;
				}
			}
		}
	 ...
	 */
	public static DictionaryBak getSingleton() {
		if (singleton == null) {
